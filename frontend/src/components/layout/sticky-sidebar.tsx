  ...
  return (
    <aside className="top-24 hidden h-fit space-y-6 lg:sticky lg:block">
      <section className="border-l border-border pl-6">
        <div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500 dark:text-slate-500">
          My Wire
        </div>
          ...
          <button
            className={`w-full border-b pb-2 text-left text-sm ${
              activeCategory === null
                ...
                : "border-border text-slate-700 dark:border-white/10 dark:text-slate-300"
            }`}
      ...
      <section className="rounded-[var(--card-radius)] border border-border bg-panel p-6 shadow-[var(--card-shadow)]">
        <div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500 dark:text-slate-500">
        ...
        <div className="space-y-4">
          {trending?.map((item) => (
            <div key={item.topic} className="border-b border-border pb-4 last:border-b-0 last:pb-0 dark:border-white/10">
              <div className="text-sm font-medium text-slate-900 dark:text-white">{item.topic}</div>
      ...
      <section className="rounded-[var(--card-radius)] border border-border bg-panel p-6 shadow-[var(--card-shadow)]">
        ...
        <div className="aspect-[4/5] border border-border bg-[radial-gradient(circle_at_top,rgba(227,97,0,0.10),transparent_35%),linear-gradient(180deg,#fff_0%,#f7f3ee_100%)] p-6 dark:border-white/10 dark:bg-[radial-gradient(circle_at_top,rgba(227,97,0,0.15),transparent_35%),linear-gradient(180deg,#121212_0%,#0a0a0a_100%)]">
          ...
          <div className="mt-8 font-heading text-5xl font-bold leading-none tracking-[-0.07em] text-heading dark:text-white">
            Pure signal
          </div>
          ...
          <div className="mt-8 space-y-3">
            <div className="flex items-center justify-between text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-slate-500">
              ...
              {topStories.length ? (
                topStories.map((story) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-sm font-medium text-slate-900 dark:text-slate-100">{story.title}</div>
                  </div>
                ))
              ) : null}
            </div>
            <div className="space-y-2">
              {topSources.length ? (
                topSources.map((source) => (
                  <div
                    key={source.slug}
                    className="flex items-center justify-between border border-border bg-white/70 px-3 py-2 text-sm text-slate-800 dark:border-white/10 dark:bg-white/5 dark:text-slate-200"
                  >
                    <span>{source.name}</span>
                    <span className="text-xs text-slate-500 dark:text-slate-500">{source.source_type}</span>
                  </div>
                ))
              ) : (
                <div className="border border-dashed border-border px-3 py-5 text-sm text-slate-500 dark:border-white/10 dark:text-slate-400">
